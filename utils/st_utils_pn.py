...
def select_target_by_conf(trgt_train_loader, threshold, model=None):
    ...
    return np.array(pc_list), np.array(seed_list), np.array(part_list), np.array(pseudo_label_list), np.array(gt_list)


class DataLoadST(Dataset):
    def __init__(self, data, partition='train'):
        self.partition = partition
        self.pc, self.seeds, self.parts, self.label, self.gt_label = data
    ...
    def __getitem__(self, item):
        ...
        data_dic = {'cloud': pointcloud, 'seeds': seeds, 'input': parts, 'label': label}
        return data_dic
