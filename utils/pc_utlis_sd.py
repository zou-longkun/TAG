...
def Subsampler(data, N, M, relative=True):
    ...
    tree = KDTree(data_out['cloud_ori'])

    dist, idx = tree.query(tinput, M)
    data_out['patch'] = data_out['cloud_ori'][idx]
    data_out['seeds'] = tinput
    tinput = np.tile(np.expand_dims(tinput, 1), (1, M, 1))
    if relative:
        data_out['patch'] = data_out['patch'] - tinput
...
def Subsampler_fps(data, N, M, relative=True):
    """
    :param data: Data Dict
    :param N: Patch Num
    :param M: Point Num per patch
...
def build_correspondence(data, N, M, relative):
    """
    N：patch_num
    M: patch_size
    """
    ...
    data_out['direc'] = data_out['direc'][indices, :]
    data_out['dist'] = data_out['dist'][indices]
    tinput = data_out['input'][indices, :]
    tree = KDTree(data_out['cloud_ori'])
    _, idx = tree.query(tinput, M)

    data_out['patch'] = data_out['cloud_ori'][idx]
    data_out['seeds'] = tinput

    tree = KDTree(data_out['input'])
    _, idx = tree.query(tinput, N * 4)
    data_out['dense_patch'] = data_out['input'][idx]
